...
__attribute__((weak))
const keycode_string_name_t *keycode_string_names_data_user = NULL;
...
static const char *search_common_names(uint16_t keycode) {
  ...
  for (int_fast16_t offset = 0; offset < ARRAY_SIZE(common_names);
       offset += 4) {
 ...
 */
static const char *search_table(const keycode_string_name_t *data,
                                uint16_t size, uint16_t keycode) {
  if (data != NULL) {
